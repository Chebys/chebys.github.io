...
function* genFiles(map){
	for(let [name, blob] of Object.entries(map))
		yield new File([blob], name)
}
...
function packMap(map){ //map为文件名到blob的映射
	return pack({[Symbol.iterator]: ()=>genFiles(map)})
}
...
function concat(meta, blob){ //meta为可JSON序列化的对象
    ...
    let metablob=new Blob([str])
    return new Blob([metablob, '\0', blob])
}
